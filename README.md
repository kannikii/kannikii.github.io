...
이미지는 두 종류다. 개발 화면을 흑백으로 옮긴 **SVG 목업**과, 실제 사진 · 화면 캡처 ·
상장 스캔본을 구운 **JPEG**이다.

**SVG 목업만 흑백이다.** 직접 찍거나 캡처한 것은 원본 색을 그대로 살린다. 서비스 화면과
상패는 색이 있어야 무엇인지 알아볼 수 있고, 회색으로 깔면 전부 같은 그림이 된다.
사이트 자체의 색(글자 · 배경 · 선)은 여전히 흑 · 백 · 회색만 쓴다.
둥근 모서리와 이모지를 쓰지 않는 규칙도 그대로다.
...
| LAB 카드 (목업) | `assets/images/lab/*-16x10.svg` | 940 × 588 | 에디터 · 터미널 · DFA · diff |
| LAB 카드 (캡처) | `assets/images/lab/*-16x10.jpg` | 1200 × 750 | 리드미에 화면이 있는 저장소 3개 |
...
- 사이트의 색(글자 · 배경 · 선 · 목업 SVG)은 흑 · 백 · 회색만 쓴다. 포인트 컬러를 넣지 않는다.
  직접 찍은 사진과 화면 캡처만 원본 색을 그대로 둔다.
