...
def build_writing():
    ...
    posts = [
        ('jwt-session-security', 'Auth · 보안', ['JWT 토큰 vs', '세션 및 보안'],
         ...
         'notion.so'),
        ('payment-idempotency', '결제 · 동시성', ['결제 구현에서의', '멱등성과 동시성 문제'],
         [('p', '요청이 실패한 건지 응답만 유실된'), ('p', '건지 서버가 스스로 걸러내야 했다.'),
          ('code', 'UNIQUE KEY uk_idem (user_id, idem_key)'),
          ('li', '멱등키 · TTL 24시간'), ('li', 'Check-then-act 경합'),
          ('li', '배타 락 대신 유니크 인덱스')],
         'notion.so'),
        ('horizontal-scaling', '인프라 · 확장', ['수평 확장'],
         [('p', 'forward 와 backward 사이에서'), ('p', '히트맵 결과가 섞여 나왔다.'),
          ('code', 'docker compose up --scale api=3'),
          ('li', '무상태 · 세션 외부화'), ('li', 'GIL — 스레드 대신 컨테이너'),
          ('li', '남은 과제 · ALB · Auto Scaling')],
         'notion.so'),
    ]
